...
export class FamilyService {
  ...
  async createInvitation(learnerId: string, guardianEmail: string): Promise<InvitationResponse> {
    ...
    const created = await this.prisma.$transaction(async (tx) => {
      ...
      const learner = await tx.user.findUnique({
        where: { id: learnerId },
        select: { role: true, email: true, loginName: true },
      });
      if (!learner || learner.role !== 'learner') {
        throw new AppError('FORBIDDEN', 403, 'Solo un alumno puede iniciar este flujo.');
      }
      if (learner.loginName !== null || !learner.email) {
        throw new AppError(
          'FORBIDDEN',
          403,
          'Tu cuenta ya es administrada por un apoderado y no usa invitaciones.',
        );
      }
  ...
  async acceptInvitation(
    ...
    code: string,
  ): Promise<{ learnerId: string; learnerName: string }> {
  ...
  async onboardingStatus(learner: SessionUser): Promise<OnboardingStatus> {
    ...
    const learningConsents = REQUIRED_LEARNING_CONSENTS.every((purpose) =>
    ...
    return {
      ...
      invitation: invitation
        ...
        : null,
      readyToEnroll:
        ageBandKnown && (!minor || (link !== null && learningConsents && assent !== null)),
    };
  }

  async grantConsents(
    guardianId: string,
    request: GrantConsentsRequest,
    retry = true,
        ...
        for (const purpose of [...new Set(request.purposes)]) {
          await tx.consentGrant.updateMany({
            where: {
              ...
              purpose,
              status: 'granted',
              noticeVersion: { not: ACTIVE_CONSENT_NOTICE_VERSION },
            },
  ...
  async revokeConsent(
    guardianId: string,
    learnerId: string,
    purpose: GrantConsentsRequest['purposes'][number],
  ...
  async recordAssent(learnerId: string): Promise<{ ok: true }> {
    await this.prisma.$transaction(async (tx) => {
      await lockLearnerPolicy(tx, learnerId);
      const existing = await tx.youthAssent.findFirst({
        where: { learnerId, noticeVersion: ACTIVE_ASSENT_NOTICE_VERSION, revokedAt: null },
      });
      if (!existing) {
        await tx.youthAssent.create({
          data: { learnerId, noticeVersion: ACTIVE_ASSENT_NOTICE_VERSION },
        });
        await this.outboxService.emitInTx(tx, {
          aggregateType: 'learner',
          aggregateId: learnerId,
          eventType: 'assent.recorded',
          payload: { noticeVersion: ACTIVE_ASSENT_NOTICE_VERSION },
        });
        await this.auditService.recordInTx(tx, {
          actorId: learnerId,
  ...
  async revokeCurrentAssent(learnerId: string): Promise<{ revoked: true }> {
    await this.prisma.$transaction(async (tx) => {
      ...
      await this.outboxService.emitInTx(tx, {
        aggregateType: 'learner',
        aggregateId: learnerId,
        eventType: 'assent.revoked',
        payload: {},
      });
      await this.auditService.recordInTx(tx, {
        actorId: learnerId,
        action: 'assent.revoked',
        objectType: 'learner',
        objectId: learnerId,
      });
    });
  ...
  async revokeOwnLink(guardianId: string, learnerId: string): Promise<{ revoked: true }> {
    await this.prisma.$transaction(async (tx) => {
      ...
      await tx.consentGrant.updateMany({
        where: { learnerId, grantedById: guardianId, status: 'granted' },
        data: { status: 'revoked', revokedAt: now },
      });
      const remainingGuardians = await tx.guardianLearnerLink.count({
